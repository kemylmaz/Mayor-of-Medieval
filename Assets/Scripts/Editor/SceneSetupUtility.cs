...
namespace MayorOfMedieval.EditorUtils
{
    ...
    public static class SceneSetupUtility
    {
        ...
        private static void CleanUp()
        {
            ...
            // Old Faz-1/2 pads lived under WORLD > Buildings / Decorations.
        ...
        {
            public Material Wood, Stone, Leaf, Animal, Grain, Bread, Pad, Wall, Roof, Water, Player, Worker, Customer, Dirt;
        }
        ...
        private static Materials CreateMaterials()
        {
            ...
            {
                Wood = Mat("M_Wood", new Color(0.45f, 0.30f, 0.16f)),
                Stone = Mat("M_Stone", new Color(0.62f, 0.62f, 0.66f)),
                Leaf = Mat("M_Leaf", new Color(0.22f, 0.55f, 0.24f)),
                Animal = Mat("M_Animal", new Color(0.92f, 0.88f, 0.82f)),
                Grain = Mat("M_Grain", new Color(0.90f, 0.76f, 0.30f)),
                Bread = Mat("M_Bread", new Color(0.78f, 0.55f, 0.25f)),
                ...
                Water = Mat("M_Water", new Color(0.30f, 0.60f, 0.85f)),
        ...
        {
            public GameObject Tree, Rock, Animal, Worker, Customer;
        ...
        private static Prefabs CreatePrefabs(Materials m)
        {
            ...
            p.LumberCamp = SavePrefab(BuildProduction(m, "LumberCamp", ResourceType.Wood, p.Worker), "P_LumberCamp");
            p.Quarry = SavePrefab(BuildProduction(m, "Quarry", ResourceType.Stone, p.Worker), "P_Quarry");
            ...
            p.Mill = SavePrefab(BuildMill(m), "P_Mill");
        ...
        private static GameObject Cube(string name, Transform parent, Vector3 pos, Vector3 scale, Material mat, bool collider = false)
        {
            ...
            return Shape(go, name, parent, pos, scale, mat, collider);
        }
        ...
        private static GameObject BuildAnimal(Materials m)
        {
            ...
            Cube("Body", root.transform, new Vector3(0f, 0.6f, 0f), new Vector3(0.55f, 0.5f, 0.95f), m.Animal);
            Sphere("Head", root.transform, new Vector3(0f, 0.85f, 0.6f), new Vector3(0.42f, 0.42f, 0.42f), m.Animal);
            Cube("LegA", root.transform, new Vector3(0.2f, 0.18f, 0.3f), new Vector3(0.12f, 0.36f, 0.12f), m.Wood);
            Cube("LegB", root.transform, new Vector3(-0.2f, 0.18f, 0.3f), new Vector3(0.12f, 0.36f, 0.12f), m.Wood);
            Cube("LegC", root.transform, new Vector3(0.2f, 0.18f, -0.3f), new Vector3(0.12f, 0.36f, 0.12f), m.Wood);
            Cube("LegD", root.transform, new Vector3(-0.2f, 0.18f, -0.3f), new Vector3(0.12f, 0.36f, 0.12f), m.Wood);
        ...
        private static Stockpile AddStockpile(GameObject parent, string name, Vector3 pos, ResourceType type,
            int capacity, bool withdraw, bool deposit)
        {
        ...
        private static ServiceCounter AddCounter(GameObject parent, Vector3 pos, Vector3 queueDir, params ResourceType[] goods)
        {
            ...
            SetPrivate(counter, "queueDirection", queueDir);
            return counter;
        }
        ...
        private static GameObject BuildMarket(Materials m)
        {
            ...
            AddCounter(root, new Vector3(0f, 0f, -2.1f), new Vector3(0f, 0f, -1f), ResourceType.Wood, ResourceType.Stone);
            Label(root.transform, new Vector3(0f, 2.5f, 0f), "PAZAR", 4f);
            return root;
        }

        private static GameObject BuildProduction(Materials m, string name, ResourceType type, GameObject workerPrefab)
        {
            GameObject root = BuildHut(name, m, new Vector3(2.2f, 1.4f, 2.2f));

            Stockpile pile = AddStockpile(root, "Stockpile", new Vector3(-1.9f, 0f, 0f), type, 20, true, false);

            WorkerStation station = root.AddComponent<WorkerStation>();
            SetPrivate(station, "workerPrefab", workerPrefab);
            SetPrivate(station, "harvestType", type);
            SetPrivate(station, "stockpile", pile);
            SetPrivate(station, "padOffset", new Vector3(2.2f, 0f, 0f));

            Label(root.transform, new Vector3(0f, 2.4f, 0f),
                type == ResourceType.Wood ? "ODUNCU" : "TAS OCAGI", 3.5f);
            return root;
        }
        ...
        private static GameObject BuildFarm(Materials m, GameObject workerPrefab)
        {
            ...
            Stockpile pile = AddStockpile(root, "MeatPile", new Vector3(-2.3f, 0f, 0f), ResourceType.Meat, 16, true, false);

            WorkerStation station = root.AddComponent<WorkerStation>();
            SetPrivate(station, "workerPrefab", workerPrefab);
            SetPrivate(station, "harvestType", ResourceType.Meat);
            SetPrivate(station, "stockpile", pile);
            SetPrivate(station, "padOffset", new Vector3(2.6f, 0f, 0f));

            AddCounter(root, new Vector3(0f, 0f, -2.3f), new Vector3(0f, 0f, -1f), ResourceType.Meat);
        ...
        private static GameObject BuildCropField(Materials m)
        {
            ...
            Stockpile pile = AddStockpile(root, "GrainPile", new Vector3(-3.2f, 0f, 0f), ResourceType.Grain, 20, true, false);
            ...
            AddCounter(root, new Vector3(0f, 0f, -2.9f), new Vector3(0f, 0f, -1f), ResourceType.Grain);
        ...
        private static GameObject BuildWell(Materials m)
        {
            ...
            Label(root.transform, new Vector3(0f, 2.5f, 0f), "KUYU", 3.5f);
            return root;
        }

        private static GameObject BuildMill(Materials m)
        {
            ...
            // Kept well apart so the player deliberately drops grain on one side and
            // collects bread on the other, instead of standing between both.
            Stockpile grainIn = AddStockpile(root, "GrainInput", new Vector3(-2.6f, 0f, 2.4f), ResourceType.Grain, 20, false, true);
            Stockpile breadOut = AddStockpile(root, "BreadOutput", new Vector3(-2.6f, 0f, -2.4f), ResourceType.Bread, 20, true, false);

            Mill mill = root.AddComponent<Mill>();
            SetPrivate(mill, "grainInput", grainIn);
            SetPrivate(mill, "breadOutput", breadOut);
            SetPrivate(mill, "sails", sails.transform);

            AddCounter(root, new Vector3(0f, 0f, -2.6f), new Vector3(0f, 0f, -1f), ResourceType.Bread);
            Label(root.transform, new Vector3(0f, 3.6f, 0f), "DEGIRMEN", 3.5f);
            return root;
        }

        // ------------------------------------------------------------------ scene

        private static void SetupManagers(Prefabs prefabs)
        {
        ...
        private static void SetupWorldNodes(Transform root, Prefabs prefabs)
        {
            ...
            ScatterCluster(nodes.transform, prefabs.Tree, new Vector3(-11f, 0f, 9f), 5.5f, 9);
            ScatterCluster(nodes.transform, prefabs.Tree, new Vector3(-15f, 0f, -6f), 4.5f, 6);
            ScatterCluster(nodes.transform, prefabs.Rock, new Vector3(-6f, 0f, -13f), 4.5f, 8);
            ScatterCluster(nodes.transform, prefabs.Rock, new Vector3(-16f, 0f, 2f), 3.5f, 5);
            ScatterCluster(nodes.transform, prefabs.Animal, new Vector3(13f, 0f, -11f), 5f, 7);
            ScatterCluster(nodes.transform, prefabs.Animal, new Vector3(16f, 0f, 6f), 4f, 5);
        }

        private static void ScatterCluster(Transform parent, GameObject prefab, Vector3 center, float radius, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                go.transform.position = center + new Vector3(offset.x, 0f, offset.y);
        ...
        private static void SetupBuildPads(Transform root, Prefabs prefabs, Materials m)
        {
            ...
            MakePad(pads.transform, m, BuildingKind.Market, prefabs.Market, new Vector3(5f, 0f, 5f));
            MakePad(pads.transform, m, BuildingKind.LumberCamp, prefabs.LumberCamp, new Vector3(-6f, 0f, 6f));
            MakePad(pads.transform, m, BuildingKind.Quarry, prefabs.Quarry, new Vector3(-8f, 0f, -6f));
            MakePad(pads.transform, m, BuildingKind.Farm, prefabs.Farm, new Vector3(9f, 0f, -6f));
            MakePad(pads.transform, m, BuildingKind.CropField, prefabs.CropField, new Vector3(2f, 0f, -12f));
            MakePad(pads.transform, m, BuildingKind.Well, prefabs.Well, new Vector3(7f, 0f, -12f));
            MakePad(pads.transform, m, BuildingKind.Mill, prefabs.Mill, new Vector3(13f, 0f, 4f));
        }
        ...
        private static void SetupCustomerRoute(Transform root, Prefabs prefabs)
        {
            ...
            exit.transform.position = new Vector3(22f, 0.9f, 20f);
        ...
        private static void SetupHUD()
        {
            ...
            // ---- gold pill, top right
            GameObject goldPanel = UIPanel(canvasGo.transform, "GoldPanel",
            ...
            // ---- quest banner, top centre
            GameObject questPanel = UIPanel(canvasGo.transform, "QuestPanel",
            ...
            // ---- settings gear, top left (visual placeholder, matches the reference layout)
        ...
        private static GameObject UIPanel(Transform parent, string name, Vector2 anchor, Vector2 pos, Vector2 size, Color color)
        {
            GameObject go = UIImage(parent, name, anchor, pos, size, color);
            return go;
        }
